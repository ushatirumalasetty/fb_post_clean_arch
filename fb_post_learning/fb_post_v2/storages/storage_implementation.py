...
from fb_post_v2.models.reactions import Reactions
...
class StorageImplementation(StorageInterface):
    ...
    def validate_comment_id(self, comment_id: int):
        ...
        is_invalid_comment_id = not is_valid_comment_id
        if is_invalid_comment_id:
            raise InvalidCommentId
    
    
    def validate_comment_id_return_parent_comment_id(self, user_id: int, comment_id: int) ->int:
        comment_obj = Comment.objects.get(id=comment_id)
        parent_comment_id = comment_obj.parent_comment_id
        raise parent_comment_id
    ...
    def get_total_reaction_count(self)->int:
        return Reactions.objects.aggregate(count=Count("reaction"))
    
    
    def get_reaction_metrices(self, post_id: int)->List[ReactionMetricesDto]:
        reaction_metrices = Reactions.objects\
